...
const FastAPIConfiguration = () => {
  ...
  const [settings, setSettings] = useState<FastAPISettings>({
    api_key: '',
    account_id: '',
    gateway_url: '',
  ...
  const loadSettings = async () => {
    setIsLoading(true);
    try {
      const { data, error } = await (window as any).ezsite.apis.tablePage(FASTAPI_SETTINGS_TABLE_ID, {
        PageNo: 1,
        PageSize: 1,
        OrderByField: 'id',
        IsAsc: false,
      ...
      toast({
        title: 'Error',
        description: 'Failed to load FastAPI settings',
        variant: 'destructive'
      });
    } finally {
      setIsLoading(false);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let error;
      if (settings.id) {
        const result = await (window as any).ezsite.apis.tableUpdate(FASTAPI_SETTINGS_TABLE_ID, {
          id: settings.id,
          ...settingsData
        });
        error = result.error;
      } else {
        const result = await (window as any).ezsite.apis.tableCreate(FASTAPI_SETTINGS_TABLE_ID, {
          ...settingsData,
      ...
      toast({
        title: 'Success',
        description: 'FastAPI settings saved successfully'
      });
      ...
      toast({
        title: 'Error',
        description: 'Failed to save FastAPI settings',
        variant: 'destructive'
      });
    } finally {
      setIsSaving(false);
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      if (isValid) {
        ...
        if (settings.id) {
          await (window as any).ezsite.apis.tableUpdate(FASTAPI_SETTINGS_TABLE_ID, {
            id: settings.id,
      ...
      toast({
        ...
        description: 'Unable to connect to FastAPI. Please check your settings.',
        variant: 'destructive'
      });
    } finally {
      setIsTesting(false);
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>Configure your FastAPI connection settings</CardDescription>
          </div>
          {getStatusBadge()}
        </div>
      </CardHeader>
      <CardContent className="space-y-6">
        {connectionStatus === 'connected' && (
          <Alert className="bg-green-500/10 border-green-500/20">
            ...
            <AlertDescription className="text-green-400">
              Successfully connected to FastAPI
            </AlertDescription>
          </Alert>
        )}
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              id="api_key"
              type="password"
              placeholder="Enter your FastAPI key"
          ...
          <div className="space-y-2">
            ...
            <Input
              id="account_id"
              type="text"
              placeholder="Enter your FastAPI account ID"
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-slate-400">
              Enter the FastAPI Gateway or API endpoint URL
            </p>
          </div>
        </div>
        ...
        <Alert className="bg-blue-500/10 border-blue-500/20">
          ...
          <AlertDescription className="text-blue-300 text-sm">
            Ensure your FastAPI Gateway is running and configured to accept API connections.
